...
class VMCController:
    def __init__(self):
        ...
        # We hold this ID until the next POLL arrives (Transaction Terminator)
    ...
    def parse_vmc_data(self, cmd, payload):
        ...
        # STAGE 1: Immediate Validation (CMD 0x02)
            ...
            # If we are waiting on a BUY command, update it
            if self.pending_action_id and self.pending_action_type == 0x03:
                ...
                # We DO NOT clear pending_action_id here. We wait for POLL.
            
            # If we were just Checking (0x01)
        ...
        while True:
            ...
            if cmd == CMD_POLL:
                ...
                if next_cmd:
                    ...
                    # Store ID for linking responses in this cycle
                    ...
                    if ack and ack['cmd'] == CMD_ACK:
                        ...
                        if self.db.increment_retry(cmd_id, next_cmd['retry_count']) == 'FAILED':
                            # If ACK failed, we definitely shouldn't expect data for this ID
                            self.pending_action_id = None
                else:
